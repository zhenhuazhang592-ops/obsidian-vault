...
description: AI image generation with OpenAI, Google, DashScope, Doubao and Replicate APIs. Supports text-to-image, reference images, aspect ratios. Sequential by default; parallel generation available on request. Use when user asks to generate, create, or draw images.
---
...
| `--provider google\|openai\|dashscope\|doubao\|replicate` | Force provider (default: google) |
...
2. `--provider` specified → use it (if `--ref`, must be `google`, `openai`, or `replicate`; Doubao supports reference images via `image_url` field)
