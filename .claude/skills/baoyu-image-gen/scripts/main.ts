...
Options:
  ...
  --provider google|openai|dashscope|doubao|replicate  Force provider (auto-detect by default)
  -m, --model <id>          Model ID
...
Environment variables:
  ...
  ARK_API_KEY               Doubao ARK API key (火山引擎)
  REPLICATE_API_TOKEN       Replicate API token
  OPENAI_IMAGE_MODEL        Default OpenAI model (gpt-image-1.5)
  GOOGLE_IMAGE_MODEL        Default Google model (gemini-3-pro-image-preview)
  DASHSCOPE_IMAGE_MODEL     Default DashScope model (z-image-turbo)
  DOUBAO_IMAGE_MODEL        Default Doubao model (doubao-seedream-4-5-251128)
  DOUBAO_BASE_URL           Custom Doubao ARK endpoint (default: https://ark.cn-beijing.volces.com/api/v3)
...
function parseArgs(argv: string[]): CliArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--provider") {
      const v = argv[++i];
      if (v !== "google" && v !== "openai" && v !== "dashscope" && v !== "doubao" && v !== "replicate") throw new Error(`Invalid provider: ${v}`);
      out.provider = v;
      continue;
    }
  ...
  for (const line of lines) {
    ...
    if (trimmed.includes(":") && !trimmed.startsWith("-")) {
      ...
      if (key === "version") {
        ...
        config.default_model = { google: null, openai: null, dashscope: null, doubao: null, replicate: null };
        currentKey = "default_model";
      } else if (currentKey === "default_model" && (key === "google" || key === "openai" || key === "dashscope" || key === "doubao" || key === "replicate")) {
        const cleaned = value.replace(/['"]/g, "");
...
function detectProvider(args: CliArgs): Provider {
  ...
  const available = [hasGoogle && "google", hasOpenai && "openai", hasDashscope && "dashscope", hasDoubao && "doubao", hasReplicate && "replicate"].filter(Boolean) as Provider[];

  if (available.length === 1) return available[0]!;
  if (available.length > 1) return available[0]!;

  throw new Error(
    "No API key found. Set GOOGLE_API_KEY, GEMINI_API_KEY, OPENAI_API_KEY, DASHSCOPE_API_KEY, ARK_API_KEY, or REPLICATE_API_TOKEN.\n" +
...
async function loadProviderModule(provider: Provider): Promise<ProviderModule> {
  ...
  if (provider === "doubao") {
    return (await import("./providers/doubao")) as ProviderModule;
  }
...
async function main(): Promise<void> {
  ...
  if (!model && extendConfig.default_model) {
    ...
    if (provider === "doubao") model = extendConfig.default_model.doubao ?? null;
